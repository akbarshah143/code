  <div class="text-center mt-2">
    <h5 class="text-primary">Register New Beneficiary Form</h5>
           ...
           <input type="hidden" class="form-control"
            ...
            <div class="mb-3">
                <label class="form-label" for="name">Beneficiary Name</label>
                  ...
                  name="name" value="{{$ides->EmployeeName }}" id="name" placeholder="Employee Name" required data-parsley-minlength="6">
               @error('name')
       ...
       <label class="form-label">Beneficiary Relation</label>
         <select class="form-control select2" id='relation' name="relation" required>
           ...
           <option value="" disabled selected hidden>Select Relation with</option>
              ...
              <option value="{{ $related->id}}" @if ($loop->first) selected @endif> {{$related->RelationDesc}}</option>
             @endforeach
          </select>
         @error('relation')
       <span class="invalid-feedback" role="alert">
      ...
      <div class="mb-3">
       ...
       name="cnic" value="{{ $ides->EmployeeCNIC }}" id="cnic" placeholder="54400-0120001-1" required data-parsley-minlength="15">
             @error('cnic')
           <span class="invalid-feedback" role="alert">
         <strong>{{ $message }}</strong>
        </span>
